...
def generate(args):
    ...
    if cp_rank == 0:
        ...
        vae_scale_factor_temporal = pipe.vae.config.scale_factor_temporal if getattr(pipe, "vae", None) else 4
        num_latent_frames = (num_frames - 1) // vae_scale_factor_temporal + 1
        # add operation here
        k_idx, nk_idx, _=extract_audio_keyframes(temp_vocal_path,0.20,num_latent_frames)
        nk_idx_tensor = torch.tensor(nk_idx, device='cuda')  # 或 CPU，但需确保所有 rank 设备一致
        ...
        context_parallel_util.cp_broadcast(full_audio_emb)
        nk_idx_tensor = torch.empty(0, dtype=torch.long, device='cuda')  # 占位，形状会在广播后填充
    dist.broadcast(nk_idx_tensor, src=0)
    ...
    if stage_1 == 'at2v':
        ...
        if cp_rank == 0:
            ...
            now = datetime.datetime.now()                     # 当前日期时间（本地时区）
            time_suffix = now.strftime("%Y%m%d_%H%M%S")  # 例如 "20250325_143015"
            save_video_ffmpeg(output_tensor, os.path.join(output_dir, f"at2v_demo_1_{time_suffix}"), raw_speech_path, fps=save_fps, quality=5)
        del output
        torch_gc()
