...
class Attention(nn.Module):
    ...
    def _process_attn(self, q, k, v, shape):
        ...
        B, H, SQ, D = q.shape
        _, _, SKV, _ = k.shape
        #print(f'in process_attn:q shape:{q.shape}')
    ...
    def forward(self, x: torch.Tensor, shape=None, num_cond_latents=None, return_kv=False, num_ref_latents=None, ref_img_index=None, mask_frame_range=None, ref_target_masks=None,
                q_mask=None,cache=None,need_cache=False) -> torch.Tensor:
        """
        """
        def slice(x,token_mask,):
            #q[b,num_heads,N,head_dim]
            #toekn_mask[b,N,1]
            # print(f'q shape: {x.shape};token_mask shape:{token_mask.shape}')
            mask_flat = token_mask[0, :, 0].contiguous()
            active_indices = torch.where(mask_flat > 0.5)[0].to(x.device).long()
            x_active=torch.index_select(x, 2, active_indices).contiguous()
            s_sliced=x_active.shape[2]
        ...
        # q_active,q_sliced,q_active_indices=slice(q,q_mask)
        ...
        if num_cond_latents is not None and num_cond_latents == 1:
            ...
            # text to video
            if need_cache:
                q,_,active_indices=slice(q,q_mask)
            x = self._process_attn(q, k, v, shape)

        x_output_shape = (B, x.shape[2], C)
        x = x.transpose(1, 2) 
        x = x.reshape(x_output_shape)
        x = self.proj(x)
        
        if need_cache:
            # 确保 cache 的 batch size 与当前输入一致 (处理 CFG 情况)
            if cache.shape[0] != B:
                # cache: [1, N, C] -> [B, N, C]
                final_x = cache.repeat(B // cache.shape[0], 1, 1).clone()
            else:
                final_x = cache.clone()
            
            # 将计算出的活跃部分填回
            # final_x: [B, N, C], x_output: [B, N_active, C]
            final_x.index_copy_(1, active_indices, x).contiguous()
            x = final_x
