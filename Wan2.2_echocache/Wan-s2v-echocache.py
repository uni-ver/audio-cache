...
def s2v_echocache_forward(
        ...
        self.need_cache=(self.running_step>=self.warmup_steps and self.running_step<=args.sample_steps-3 and self.running_step%15!=0)
        self.need_update_cache=(self.running_step>=self.warmup_steps-2 and self.running_step<=args.sample_steps-3)
        ...
        self.cnt+=1
        return [u.float() for u in x],counter.get_total_flops()/1e12
...
def s2v_generate(
        ...
        self.key_frame_indices, self.non_key_frame_indices, self.frame_energies=extract_audio_keyframes(audio_path,selected_ratio,latent_T,None,args.random_select,reverse=args.reverse)
        # print(f'non_key_frame_indices:{self.non_key_frame_indices}')
        _, self.k_non_key_frame_indices, _=extract_audio_keyframes(audio_path,0.8,latent_T,None,args.random_select)
        ...
        with (
            ...
            for r in range(num_repeat):
                ...
                with flop_counter:
                    for i, t in enumerate(tqdm(timesteps)):
                        ...
                        noise_pred_cond,flopcount = self.noise_model(
                            latent_model_input, 
                            t=timestep, 
                            **arg_c,
                            current_step=t, 
                        ...
                        totalcount+=flopcount
                        if guide_scale > 1:
                            noise_pred_uncond,flopcount= self.noise_model(
                                latent_model_input, 
                                t=timestep, 
                                **arg_null,
                                current_step=t, 
                        ...
                        if self.rank == 0:
                            ...
                            print(f"Step {i} cumulative FLOPs: {total_flops / 1e12:.2f} TFLOPs")
            ...
            if self.rank == 0:
                ...
                print(f'Total Inference FLOPs(hook): {totalcount:.2f} TFLOPs')
...
def generate(args):
    ...
    logging.info("Creating WanS2V pipeline with EchoCache.")
    wan_s2v = wan.WanS2V(
        config=cfg,
    ...
    wan_s2v.noise_model.__class__.forward = s2v_echocache_forward
    wan_s2v.noise_model.__class__.enable_teacache = args.enable_teacache
    ...
    wan_s2v.noise_model.__class__.warmup_steps=args.warmup_steps
