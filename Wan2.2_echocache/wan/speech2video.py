...
from .audiovis import visualize_audio_embeddings
import numpy as np
import torch
...
class WanS2V:
    ...
    def encode_audio(self, audio_path, infer_frames):
        ...
        #prefix = os.path.basename(audio_path).split('.')[0]
        #visualize_audio_embeddings(audio_embed_bucket, prefix=prefix)
    ...
    def generate(
        ...
        self.key_frame_indices, self.non_key_frame_indices, self.frame_energies=extract_audio_keyframes(audio_path,1,latent_T,sample_fps=16)#这个参数是确定的，但是存储在generate.py中的cfg.sample_fps，这里写死是因为不想传参
        # print(f'non_key_frame_indices:{self.non_key_frame_indices}')
        _, self.k_non_key_frame_indices, _=extract_audio_keyframes(audio_path,1,latent_T,sample_fps=16)
        audio_emb, nr = self.encode_audio(audio_path, infer_frames=infer_frames)
        # print(f"Audio emb shape: {audio_emb.shape}, dtype: {audio_emb.dtype}")
        # print(f"Audio emb stats: mean={audio_emb.mean().item():.4f}, std={audio_emb.std().item():.4f}, min={audio_emb.min().item():.4f}, max={audio_emb.max().item():.4f}")
        ...
        with (
            ...
            for r in range(num_repeat):
                ...
                # print("=== AudioInjector structure ===")
                # print("audio_injector type:", type(self.noise_model.audio_injector))
                # print("injector length:", len(self.noise_model.audio_injector.injector))
                # print("injector_pre_norm_feat length:", len(self.noise_model.audio_injector.injector_pre_norm_feat))
                # if hasattr(self.noise_model.audio_injector, 'injector_adain_layers'):
                #     print("injector_adain_layers length:", len(self.noise_model.audio_injector.injector_adain_layers))
                # if hasattr(self.noise_model.audio_injector, 'injector_adain_output_layers'):
                #     print("injector_adain_output_layers length:", len(self.noise_model.audio_injector.injector_adain_output_layers))
                
                # 尝试打印第一个注入层的参数（如果存在）
                # if len(self.noise_model.audio_injector.injector) > 0:
                #     first_inj = self.noise_model.audio_injector.injector[0]
                #     # print("First injector type:", type(first_inj))
                #     for name, param in first_inj.named_parameters():
                #         #print(f"  {name}: shape={param.shape}, mean={param.data.mean():.6f}, std={param.data.std():.6f}")
                #     for i, inj in enumerate(self.noise_model.audio_injector.injector):
                #         w = inj.o.weight
                #         # print(f"Injector {i} o.weight: mean={w.mean():.6f}, std={w.std():.6f}, max={w.max():.6f}, min={w.min():.6f}")
                # else:
                #     # print("injector list is empty!")
                ...
                with flop_counter:
                    ...
                    print(f"Total Inference FLOPs: {flop_counter.get_total_flops() / 1e12:.2f} TFLOPs")
                if offload_model:
