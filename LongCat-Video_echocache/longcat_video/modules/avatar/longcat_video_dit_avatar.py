    ...
    def forward(
        ...
        if need_cache:
            ...
            if q_mask is not None:
                if q_mask.shape[0] > B:
                    current_q_mask = q_mask[:B]
                elif q_mask.shape[0] < B:
                    current_q_mask = q_mask.repeat(B, 1, 1, 1)
                else:
                    current_q_mask = q_mask
            ...
            if cache is not None:
                if cache.shape[0] > B:
                    ...
                    current_cache = cache.repeat(B, 1, 1)
                else:
                    current_cache = cache

        # --- 以下进入计算流程 ---
        # print(f'LongCatAvatarSinglestreamBlock: x shape: {x.shape}')
        # print(f'latent shape={latent_shape}')#(14,30,52)
        ...
        if kv_cache is not None:
            ...
            attn_outputs = self.attn(x_m, shape=latent_shape, num_cond_latents=num_cond_latents, return_kv=return_kv, num_ref_latents=num_ref_latents, \
                                                            ref_img_index=ref_img_index, mask_frame_range=mask_frame_range, ref_target_masks=token_ref_target_masks,
                                                            q_mask=current_q_mask,cache=current_cache,need_cache=need_cache)
        ...
        with amp.autocast(device_type='cuda', dtype=torch.float32):
            x = x + (gate_msa * x_s.reshape(B, -1, N//T, C)).reshape(B, -1, C) # [B, N, C]
        x = x.to(x_dtype)
        ...
        if not skip_crs_attn:
            ...
            with amp.autocast(device_type='cuda', dtype=torch.float32):  
                audio_output_noise = modulate_fp32(self.mod_norm_attn, audio_output_noise.reshape(B, T-num_cond_latents, -1, C), audio_shift_mca, audio_scale_mca).view(B, -1, C)
                audio_add_x = (audio_gate_mca * audio_output_noise.reshape(B, T-num_cond_latents, -1, C)).reshape(B, -1, C) # [B, N, C]
        ...
        # ffn with modulation
        x_m = modulate_fp32(self.mod_norm_ffn, x.reshape(B, -1, N//T, C), shift_mlp, scale_mlp).reshape(B, -1, C)
        if need_cache and current_cache is not None and current_q_mask is not None:
            mask_flat = current_q_mask[0].view(-1) 
            active_indices = torch.where(mask_flat > 0.5)[0]
            x_m=torch.index_select(x_m, 1, active_indices).contiguous()
        x_s = self.ffn(x_m)
        if need_cache and current_cache is not None and current_q_mask is not None:
            # 将计算出的活跃部分填回
            # final_x: [B, N, C], x_output: [B, N_active, C]
            final_x=current_cache.clone()
            final_x.index_copy_(1, active_indices, x_s).contiguous()
            x_s= final_x
        with amp.autocast(device_type='cuda', dtype=torch.float32):
            x = x + (gate_mlp * x_s.reshape(B, -1, N//T, C)).reshape(B, -1, C) # [B, N, C]
        x = x.to(x_dtype)
        if need_cache and current_cache is not None and current_q_mask is not None:
            # 确保 mask 类型对齐 (防止之前的 dtype 报错)
            mask_val = current_q_mask.squeeze(-1)
            x = mask_val * x + current_cache * (1.0 - mask_val)
...
class LongCatVideoAvatarTransformer3DModel(
    ...
    def __init__(
        ...
        #init cache
        self.cache=[None for i in range(depth)]
    ...
    def forward(
        ...
        # print(f'embedded hidden states shape:{hidden_states.shape}')
        with amp.autocast(device_type='cuda', dtype=torch.float32):
        ...
        for i, block in enumerate(self.blocks):
            #generating need_cache timestep and block:
            need_cache=(step>=5 and step<44 and step%10!=0)and(i>=1 and i<len(self.blocks)-1)
            # need_cache=False
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                # print(f'3D Model: hidden_states shape: {hidden_states.shape}')
                block_outputs = block(
                    hidden_states, encoder_hidden_states, t, y_seqlens,
                    (N_t, N_h, N_w), num_cond_latents, return_kv, kv_cache_dict.get(i, None), skip_crs_attn, num_ref_latents, audio_hidden_states, ref_img_index, mask_frame_range, token_ref_target_masks, human_num,
                    q_mask=q_mask,cache=self.cache[i],need_cache=need_cache
                )
            
            if return_kv:
                hidden_states, kv_cache,self.cache[i] = block_outputs
                if offload_kv_cache:
                    kv_cache_dict_ret[i] = (kv_cache[0].cpu(), kv_cache[1].cpu())
                else:
                    kv_cache_dict_ret[i] = (kv_cache[0].contiguous(), kv_cache[1].contiguous())
            else:
                hidden_states, layer_output = block_outputs
                # 更新当前层的 cache 供下一个 step 使用
                # 注意：为了节省显存并避免 B 变化，通常只存储 batch_size=1 的部分或在存储前处理
                self.cache[i] = layer_output.detach()[:1] # 只存一份，CFG 时在 Attention 内部 repeat
                if torch.isnan(self.cache[i]).any():
                    print(f"step{step},block{i},cache发现 NaN！")
