...
class LongCatVideoAvatarPipeline:
    ...
    def generate_at2v(
        ...
        #operation here
        b,c,n,h,w=latents.shape
        print(f'latent shape:{latents.shape}')
        mask=torch.ones((b*n,1,1,1),device=device).to(torch.bfloat16)
        mask[mask_indices]=0
        mask=mask.expand(b*n,h*w//4,1,1).reshape(b,n*h*w//4,1,1)
        #end operation
        if context_parallel_util.get_cp_size() > 1:
        ...
        with tqdm(total=len(timesteps), desc="Denoising") as progress_bar:
            ...
            with counter:
                for i, t in enumerate(timesteps):
                    ...
                    q_mask=torch.cat([mask]*2) if self.do_classifier_free_guidance else mask
                    ...
                    if i == len(timesteps) - 1 or (i + 1) % self.scheduler.order == 0:
                        progress_bar.update()
                print(f"Step {i} cumulative: {counter.get_total_flops() / 1e12:.2f} TFLOPs")
        self._current_timestep = None
